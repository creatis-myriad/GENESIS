...
| **Arguments**   | `{proximal\|distal}`: Variant of the score to compute, that only considers obstructions in proximal (i.e. mediastinal and lobar) or distal (i.e. segmental) arteries towards in the final score. |
| **Options**     | `-o, --obstruction-attr TEXT`: Edge attribute to use as obstruction values. <br>                                                                                                                 |
| **Examples**    | Compute proximal Mastora score for one patient: `eval-graph -g data/PERSEVERE/graphs -p '*{id}*.json' -l 0055 mastora proximal`                                                                  |

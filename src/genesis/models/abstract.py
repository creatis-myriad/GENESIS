        ...
        if any(val is not None for val in data_hparams.values()):
            ...
            if missing_required_hparams:
                ...
                if hasattr(fake_dataset, "graph_attr"):
                    ...
                    # for the changes to be reflected when accessing the individual `Data` objects
                    fake_dataset._data.graph_attr = fake_dataset.graph_attr.clip(0, 1)

                # Convert/cast the underlying `Data` objects generated by `FakeDataset` to `GraphAttrData` objects
                # so that if graph features are present, they are properly handled when batching
                data_list = [GraphAttrData.from_dict(data.to_dict()) for data in fake_dataset]

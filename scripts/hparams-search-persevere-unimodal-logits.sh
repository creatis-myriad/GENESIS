...
for model in gin; do
#  for variant in "" +vn +ef +vn+ef +lf +vn+lf +gavn; do
  # shellcheck disable=SC2043
  for variant in +gavn; do
    gnn-train hydra/launcher=joblib hydra.launcher.n_jobs=10 trainer=gpu logger=wandb \
      ...
      model/head=unimodal_logits ~model.head.num_layers \
      logger.wandb.project=GENESIS-ordinal-regression-hparams-search \

...
for experiment in gcn gat gin gin+vn gin-vcn; do
  gnn-train hydra/launcher=joblib hydra.launcher.n_jobs=10 logger=wandb trainer=gpu hparams_search=persevere_basic_gnn experiment=persevere/$experiment data/dataset/target="$TARGET" +data.on_conflict=ignore model/metrics=binary_classification >>"${LOG_DIR}/hparams_search_${experiment}_${TARGET}.log" 2>&1
done
...
gnn-train logger=wandb trainer=gpu hydra/launcher=joblib hydra.launcher.n_jobs=4 hparams_search=persevere_gps data/dataset/target="$TARGET" +data.on_conflict=ignore model/metrics=binary_classification >>"${LOG_DIR}/hparams_search_gps_${TARGET}.log" 2>&1

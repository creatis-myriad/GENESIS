...
for target in risk_ESC-2014 enzymes_elevated; do
  gnn-train -m hydra/launcher=joblib hydra.launcher.n_jobs=5 logger=wandb logger.wandb.log_model=True trainer=gpu test=True experiment=persevere/${target}/gcn,persevere/${target}/gcn+vn,persevere/${target}/gcn+gavn,persevere/${target}/gat,persevere/${target}/gat+vn,persevere/${target}/gat+gavn,persevere/${target}/gin,persevere/${target}/gin+vn,persevere/${target}/gin+gavn,persevere/${target}/gps data/split=k_fold +data.on_conflict=ignore 'data.split_idx=range(10)' >>"${LOG_DIR}/persevere-gnn-${target}.log" 2>&1
done
...
for target in troponin_elevated nt-probnp_elevated; do
  gnn-train -m hydra/launcher=joblib hydra.launcher.n_jobs=5 logger=wandb logger.wandb.log_model=True trainer=gpu test=True experiment=persevere/"${DEFAULT_HPARAMS}"/gcn,persevere/"${DEFAULT_HPARAMS}"/gcn+vn,persevere/"${DEFAULT_HPARAMS}"/gcn+gavn,persevere/"${DEFAULT_HPARAMS}"/gat,persevere/"${DEFAULT_HPARAMS}"/gat+vn,persevere/"${DEFAULT_HPARAMS}"/gat+gavn,persevere/"${DEFAULT_HPARAMS}"/gin,persevere/"${DEFAULT_HPARAMS}"/gin+vn,persevere/"${DEFAULT_HPARAMS}"/gin+gavn,persevere/"${DEFAULT_HPARAMS}"/gps data/split=k_fold data/dataset/target=${target} model/metrics=binary_classification +data.on_conflict=ignore 'data.split_idx=range(10)' >>"${LOG_DIR}/persevere-gnn-${target}.log" 2>&1
done
